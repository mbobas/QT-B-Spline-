    ...
    ui(new Ui::MainWindow)
{
    ...
    //nadajemy kolor podstawowy
     ...
     highOfBigPixel = 5; // wielkosc kliknietego piksela
...
void MainWindow::mousePressEvent(QMouseEvent *event)
{
        ...
        if(event->button() == Qt::RightButton )
        {
            ...
            // jesli liczba pikseli osiagnie 4 lub wiecej to zwiekszamy licznik lini
            if(numberOfPixels >= 3)
            {
                counterOfLines++; // licznik lini
                //numberOfPixels++; // licznik pikseli - powielony pixel - nie bedziemy go rysowac
                x[numberOfPixels] = pointX; // piksel końca nowej krzywej b-spline
...
//            }
        ...
        if (numberOfPixels>=3)
        {
            for (int  i=0; i<=counterOfLines; i++)
            {
                ...
                draw_bspline(i, i, i+1, i+1, i+2, i+2, i+3, i+3);
            }
        }
    }
    update();
...
void MainWindow::draw_bspline(int px0, int py0, int px1, int py1,int px2, int py2,int px3, int py3 )
{
    int x_1,y_1;
    ...
    for (float t=0; t<=1; t+=0.001) {
       ...
       // y_1=qPow((1-t),3)*y[py0]+3*(qPow((1-t),2))*t*y[py1]+3*(1-t)*qPow(t,2)*y[py2]+qPow(t,3)*y[py3];
        ...
        x_1=((qPow(-t,3)+3*qPow(t,2)-3*t+1)/6)*x[px0] + ((3*qPow(t,3)-6*qPow(t,2)+4)/6)*x[px1] + ((-3*qPow(t,3)+3*qPow(t,2)+3*t+1)/6)*x[px2] + (qPow(t,3)/6)*x[px3];
        y_1=((qPow(-t,3)+3*qPow(t,2)-3*t+1)/6)*y[py0] + ((3*qPow(t,3)-6*qPow(t,2)+4)/6)*y[py1] + ((-3*qPow(t,3)+3*qPow(t,2)+3*t+1)/6)*y[py2] + (qPow(t,3)/6)*y[py3];
